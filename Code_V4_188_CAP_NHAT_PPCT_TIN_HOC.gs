...
function v4162ReadWeekLastPpctByPair_(sheet, mondayYmd) {
  ...
  for (let c = 0; c < header.length - 1; c++) {
    const label = keyText_(header[c]).replace(/[^a-z0-9]+/g, '');
    if (label !== 'hoten') continue;
    const fullName = normalizeText_(header[c + 1]);
  ...
  blocks.forEach(block => {
    ...
    for (let r = 0; r < vals.length; r++) {
      ...
      if (!(ppctNumber > 0)) continue; // tự loại PĐ, ô trống, nội dung không phải PPCT chuẩn
