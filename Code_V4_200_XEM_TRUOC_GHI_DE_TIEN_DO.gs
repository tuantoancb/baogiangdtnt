// V4.191 — NHẬN ĐÚNG KHỐI GIÁO VIÊN KHI TÊN NẰM CHUNG Ô "HỌ TÊN".
// - Nhận cả hai mẫu: "Họ tên:" + ô tên kế bên và "Họ tên: Hoàng Thị Lan" trong một ô.
// - Không bỏ sót tuần 7-12.9 của cô Lan khi tính mốc tuần trước.
// - Khôi phục đúng tiến độ CT và PĐ của cô Lan ở tuần 14-19.9.
// Kế thừa V4.190 — TÁCH TUYỆT ĐỐI TIẾN ĐỘ CHÍNH KHÓA / CĐ / PĐ.
// - PĐ1, PĐ2... không còn bị đọc thành CT1, CT2... khi lấy mốc tuần trước.
// - Tiến độ PĐ được nối riêng qua các tuần; CĐ và Chính khóa giữ độc lập.
// - Khôi phục đúng trường hợp Nguyễn Thanh Tuấn - Toán 12A4: CT14 -> CT15.
// Kế thừa V4.189 — MỞ NHANH BÁO GIẢNG ĐÚNG VỊ TRÍ GIÁO VIÊN.
...
// V4.180 — FIX dò khối giáo viên + đồng bộ công tắc PĐ; kế thừa V4.179.
// V4.162 — Tiến độ PPCT đi theo Lớp + Môn khi đổi phân công; kế thừa V4.161 quản trị giáo viên.
...
function v4183FindTeacherHeaderStart_(sheet, teacherHitCol1, teacherHitRow1) {
  ...
  // V4.185: không lấy tiêu đề đầu tiên nhìn thấy trong vùng quét nữa.
  // Hai khối giáo viên nằm cạnh nhau có thể cùng lọt vào vùng quét (ví dụ Diệp ở FM, Lan ở FU).
  // Thu tất cả tiêu đề hợp lệ rồi chọn tiêu đề GẦN ô tên giáo viên nhất.
  for (let r=0; r<vals.length; r++) {
    for (let c=0; c<vals[r].length; c++) {
      ...
      if(score>=4){
        candidates.push({col:absCol,row:rowFrom+r,score:score,distance:Math.abs(absCol-teacherHitCol1)});
      }
    }
  }
  ...
  // Khoảng cách lớn thường là tiêu đề của giáo viên bên cạnh; không đoán để tránh ghi nhầm.
  if(candidates[0].distance>4) return null;
  return candidates[0].col; // 1-based cột "Ngày, thứ"
}
function findTeacherBlockStart_(sheet, fullName) {
  ...
  const candidates = {};
  [fullName, profile.fullName, teacherKey, profile.slug].forEach(function(v){
...
// V4.184: chỉ cảnh báo ghi đè khi ô đích thực sự chứa DỮ LIỆU BÁO GIẢNG.
// Không coi tiêu đề, ký tự rác/ẩn hoặc ô mẫu là dữ liệu đã ghi.
function v4184CleanCell_(value){
  return normalizeText_(String(value == null ? '' : value).replace(/[\u200B-\u200D\uFEFF]/g,''));
}
function v4184LooksLikeHeaderTarget_(vals){
  const a=vals.map(function(x){return v4184CleanCell_(x).toLowerCase();});
  const joined=a.join(' | ');
  return (a[0]==='môn' || a[0]==='môn học') &&
         a[1].indexOf('lớp')===0 &&
         (a[2].indexOf('ppct')>=0 || a[2].indexOf('tiết')>=0) &&
         (a[3].indexOf('tên bài')>=0 || joined.indexOf('công tác')>=0 || joined.indexOf('tập huấn')>=0);
}
function v4184HasRealReportData_(vals){
  vals=(vals||[]).map(v4184CleanCell_);
  if(v4184LooksLikeHeaderTarget_(vals)) return false;
  const subject=vals[0]||'', cls=vals[1]||'', ppct=vals[2]||'', lesson=vals[3]||'';
  const classLike=/^(?:10|11|12)\s*[A-ZÀ-Ỹ0-9._-]+$/i.test(cls.replace(/\s+/g,''));
  const ppctLike=/^(?:(?:CT|CD|CĐ|PĐ|PD)\s*)?\d+(?:\s*[-–]\s*\d+)?$/i.test(ppct.replace(/\s+/g,''));
  // Một dòng báo giảng thật thường có Lớp + ít nhất một nội dung khác,
  // hoặc PPCT + Tên bài. Không cảnh báo chỉ vì một ô mẫu có chữ/rác.
  if(classLike && (subject || ppct || lesson)) return true;
  if(ppctLike && lesson) return true;
  if(subject && lesson) return true;
  return false;
}
function checkExistingWrite(payload){
  ...
  const ss=SpreadsheetApp.openById(BAO_GIANG_SPREADSHEET_ID);
  const sh=ss.getSheetByName(check.sheet);
  let count=0, ranges=[], details=[];
  check.targets.forEach(function(t){
    const vals=sh.getRange(t.range).getDisplayValues()[0];
    const real=v4184HasRealReportData_(vals);
    details.push({range:t.range,existing:real,values:vals});
    if(real){count++;ranges.push(t.range);}
  });
  return {ok:true,hasExisting:count>0,count:count,ranges:ranges,details:details};
}
...
function capNhatTienDoStep2(payload) {
  try {
    ...
    const firstTarget = progressDetails && progressDetails.length ? progressDetails[0] : null;
    const targetUrl = firstTarget
      ? ('https://docs.google.com/spreadsheets/d/' + TIEN_DO_SPREADSHEET_ID + '/edit#gid=' + firstTarget.gid + '&range=' + encodeURIComponent(firstTarget.range))
      : ('https://docs.google.com/spreadsheets/d/' + TIEN_DO_SPREADSHEET_ID + '/edit');
    return {
      ok:true,
      error:'',
    ...
    return {
      ...
      details:[],
      spreadsheetId:TIEN_DO_SPREADSHEET_ID,
      targetUrl:'https://docs.google.com/spreadsheets/d/' + TIEN_DO_SPREADSHEET_ID + '/edit'
    };
  }
}
...
// V4.182: lập bản đồ ô đích chịu được cả nhãn "2", "Thứ 2", ô gộp và mẫu báo giảng mới.
function v4182DayNumFromCell_(value) {
  const s = normalizeText_(value);
  if (!s) return null;
  let m = s.match(/^(?:Thứ\s*)?([2-7])(?:\s|\(|$)/i);
  if (!m) m = s.match(/(?:^|\s)([2-7])(?:\s|\(|$)/);
  return m ? Number(m[1]) : null;
}
function v4182SessionKey_(value) {
  const s = normalizeText_(value).toLowerCase();
  return s.indexOf('chiều') >= 0 ? 'Chiều' : 'Sáng';
}
function v4136BuildTargetRowMap_(sheet, blockStartZero) {
  ...
  [['Sáng', sections.morning], ['Chiều', sections.afternoon]].forEach(pair => {
    ...
    let currentDay = null;
    vals.forEach((row, i) => {
      const parsedDay = v4182DayNumFromCell_(row[0]);
      if (parsedDay) currentDay = parsedDay;
      const period = Number(String(normalizeText_(row[1])).replace(',', '.'));
      if (currentDay && Number.isFinite(period) && period >= 1 && period <= 5) {
        map[session + '_' + currentDay + '_' + period] = firstDataRow + i;
      }
    });

    // Fallback theo đúng cấu trúc mẫu chuẩn: Thứ 2..7, mỗi ngày 5 dòng.
    // Chỉ lấp các khóa còn thiếu, không ghi đè kết quả đọc được từ sheet.
    for (let day = 2; day <= 7; day++) {
      for (let period = 1; period <= 5; period++) {
        const key = session + '_' + day + '_' + period;
        if (!map[key]) map[key] = firstDataRow + (day - 2) * 5 + (period - 1);
      }
    }
  });
  return map;
...
function preflightBaoGiang(payload) {
  ...
  records.forEach(rec => {
    const row = rowMap[v4182SessionKey_(rec.session) + '_' + rec.dayNum + '_' + rec.period];
...
const V4163_ADMIN_SPREADSHEET_ID = '1m9DC5wKFSsZQI2Tcv-Q8_d8Gkuz9dnk92u1mp2qc7rw';
...
function v4161EnsureAccessSheet_() {
  const ss = SpreadsheetApp.openById(V4163_ADMIN_SPREADSHEET_ID);
...
// ===== V4.162: TIẾN ĐỘ ĐI THEO LỚP + MÔN, KHÔNG ĐI THEO GIÁO VIÊN =====
// Khi lớp được chuyển từ giáo viên A sang giáo viên B, PPCT của B tiếp tục từ PPCT cuối
// đã dạy của chính Lớp + Môn + loại Chính khóa/Chuyên đề trong Lịch báo giảng các tuần trước.
// Không sửa dữ liệu tuần cũ; chỉ đổi cách tìm mốc PPCT để bắt đầu tuần hiện tại.
const V4162_REPORT_SCAN_MAX_ROWS = 120;

function v4162SinglePrimarySubject_(fullName) {
  const raw = normalizeText_(getTeacherDirectoryInfo_(fullName).subject || '');
  if (!raw) return '';
  const parts = raw.split(/[;,]+/).map(normalizeText_).filter(Boolean);
  return parts.length === 1 ? parts[0] : '';
}

// Đọc một tuần Báo giảng chỉ bằng MỘT lần getRange cho toàn bảng,
// rồi lấy PPCT cuối theo từng Lớp + Môn + track trên toàn bộ giáo viên.
function v4162ReadWeekLastPpctByPair_(sheet, mondayYmd) {
  const result = {};
  if (!sheet) return result;
  const lastCol = Math.max(1, sheet.getLastColumn());
  const rowCount = Math.min(Math.max(2, sheet.getLastRow()), V4162_REPORT_SCAN_MAX_ROWS);
  const vals = sheet.getRange(1, 1, rowCount, lastCol).getDisplayValues();
  const header = vals[1] || [];
  const blocks = [];

  for (let c = 0; c < header.length; c++) {
    const rawHeader = normalizeText_(header[c]);
    if (!rawHeader) continue;

    // Hai mẫu đang tồn tại trong Báo giảng:
    // 1) FU2 = "Họ tên:", FV2 = "Hoàng Thị Lan"
    // 2) FU2 = "Họ tên: Hoàng Thị Lan" (ô gộp/ghi chung)
    // Trước V4.191 chỉ nhận mẫu 1 nên bỏ qua trọn tuần 7-12.9 của cô Lan.
    const colonIndex = rawHeader.indexOf(':');
    const labelText = colonIndex >= 0 ? rawHeader.slice(0, colonIndex) : rawHeader;
    const label = keyText_(labelText).replace(/[^a-z0-9]+/g, '');
    if (label !== 'hoten' && label !== 'hovaten') continue;

    const inlineName = colonIndex >= 0 ? normalizeText_(rawHeader.slice(colonIndex + 1)) : '';
    const fullName = inlineName || normalizeText_(header[c + 1]);
    if (!fullName) continue;
    blocks.push({startColZero:c, fullName:fullName, primarySubject:v4162SinglePrimarySubject_(fullName)});
  }
  ...
  blocks.forEach(block => {
    ...
    for (let r = 0; r < vals.length; r++) {
      const row = vals[r] || [];
      const seven = row.slice(block.startColZero, block.startColZero + 7);
      while (seven.length < 7) seven.push('');
      const rowText = seven.map(normalizeText_).join(' | ').toUpperCase();
      if (rowText.indexOf('BUỔI SÁNG') >= 0) { currentSession = 'Sáng'; currentDay = null; }
      if (rowText.indexOf('BUỔI CHIỀU') >= 0) { currentSession = 'Chiều'; currentDay = null; }

      const dayCell = normalizeText_(seven[0]);
      const dm = dayCell.match(/^([2-7])(?:\s|\(|$)/);
      if (dm) currentDay = Number(dm[1]);

      const period = Number(String(seven[1] || '').replace(',', '.'));
      const cls = normalizeText_(seven[3]);
      if (!v4146ClassGrade_(cls)) continue;
      if (!Number.isFinite(period) || period < 1 || period > 5) continue;

      const rawPpct = normalizeText_(seven[4]);
      const ppctNumber = v4135PpctNumber_(rawPpct);
      if (!(ppctNumber > 0)) continue;

      const rawSubject = normalizeText_(seven[2]) || block.primarySubject;
      if (!rawSubject) continue;
      const subjectInfo = v4135SubjectTrackInfo_(rawSubject);
      const ppctTrack = v4135TrackFromPpct_(rawPpct);

      // V4.190: nhãn PPCT là nguồn xác định track mạnh nhất.
      // Trước đây nhánh này chỉ phân biệt CĐ và Chính khóa, nên PĐ1 bị rơi vào
      // Chính khóa như số 1. Hậu quả: 12A4 kết thúc CT14 nhưng tuần sau chạy CT2.
      // Giữ đủ ba track để các bộ đếm lịch sử hoàn toàn độc lập.
      let track = ppctTrack !== V4186_TRACK_REGULAR ? ppctTrack : subjectInfo.track;
      if (track !== V4186_TRACK_ELECTIVE && track !== V4186_TRACK_REMEDIAL) {
        track = V4186_TRACK_REGULAR;
      }
      const baseSubject = subjectInfo.baseSubject;
      if (!baseSubject || baseSubject === 'Chưa xác định môn') continue;
      const subject = track === V4186_TRACK_ELECTIVE
        ? (baseSubject + ' CĐ')
        : (track === V4186_TRACK_REMEDIAL ? (baseSubject + ' PĐ') : baseSubject);
      const pairKey = v4127PairKey_(cls, subject, track);

      // Mốc thời gian trong tuần: ngày -> sáng/chiều -> tiết -> dòng.
      const stamp = (Number(currentDay || 9) * 100000) + ((currentSession === 'Sáng' ? 0 : 1) * 10000) + (period * 100) + (r + 1);
      const prev = latest[pairKey];
      if (!prev || stamp > prev.stamp || (stamp === prev.stamp && ppctNumber > prev.ppct)) {
        latest[pairKey] = {stamp:stamp, ppct:ppctNumber};
      }
    }
  });

  Object.keys(latest).forEach(k => { result[k] = latest[k].ppct; });
  return result;
}

// GHI ĐÈ hàm cũ: teacherKey được giữ trong chữ ký để tương thích các chỗ gọi cũ,
// nhưng mốc lịch sử không còn phụ thuộc giáo viên.
function readLastPpctBeforeSheet_(teacherKey, currentSheetName) {
  const ordered = orderedWeekSheets_();
  const current = ordered.find(x => x.name === currentSheetName);
  if (!current) return {};
  ...
  ordered.filter(x => x.time < current.time).sort((a,b) => a.time - b.time).forEach(w => {
    const sh = ss.getSheetByName(w.name);
    if (!sh) return;
    const one = v4162ReadWeekLastPpctByPair_(sh, w.monday || '');
    Object.keys(one).forEach(pairKey => {
      const n = Number(one[pairKey]);
      if (Number.isFinite(n) && n > 0) result[pairKey] = n;
    });
...
// Hàm kiểm tra nhanh trong Apps Script: truyền tuần Báo giảng hiện tại và xem mốc toàn trường.
// Ví dụ V4162_KIEM_TRA_MOC_TIEN_DO('31.8-5.9') sẽ trả map Lớp+Môn -> PPCT cuối trước tuần đó.
function V4162_KIEM_TRA_MOC_TIEN_DO(currentSheetName) {
  const name = normalizeText_(currentSheetName) || (v4158LatestReportSheet_().name || '');
  const map = readLastPpctBeforeSheet_(FIXED_TEACHER_KEY, name);
  return {ok:true, currentSheet:name, pairs:Object.keys(map).length, lastByPair:map};
}


// V4.163: Bảng quản trị giáo viên được tách khỏi file Báo giảng.
// Chạy hàm này để kiểm tra nhanh app đang trỏ đúng file quản trị riêng.
function V4163_KIEM_TRA_FILE_QUAN_TRI() {
  const ss = SpreadsheetApp.openById(V4163_ADMIN_SPREADSHEET_ID);
  const sh = v4161EnsureAccessSheet_();
  return {
    ok:true,
    ...
    teachers:Math.max(0, sh.getLastRow()-1),
    url:'https://docs.google.com/spreadsheets/d/' + ss.getId() + '/edit'
  };
}
...
const VERCEL_API_ACTIONS = {
  ...
  'xemTruocCapNhatTienDo': xemTruocCapNhatTienDo,
  'getPrintableBaoGiang': getPrintableBaoGiang,
  'kiemTraTienDoDaCapNhat': kiemTraTienDoDaCapNhat,
  'auditProgressContinuity': auditProgressContinuity,
  ...
  'getPdRegularToggle': getPdRegularToggle,
  'setPdRegularToggle': setPdRegularToggle
};
...
// - Khi BẬT: các nhãn PĐ / PD / Phụ đạo trong TKB được đổi thành môn chính khóa
//   tương ứng trước khi tính PPCT, vì vậy dùng chung nguồn PPCT chính khóa và lấy CT kế tiếp.
// - Khi TẮT: các tiết PĐ được bỏ qua khi lên Báo giảng / tính tiến độ; không chiếm CT.
// - Trạng thái lưu riêng theo mã giáo viên trong Script Properties.
// ============================================================================
const V4179_PD_PROP_PREFIX = 'V4179_PD_AS_REGULAR_';

function v4179PdPropKey_(teacherKey) {
  const key = resolveTeacherKey_(teacherKey);
  return V4179_PD_PROP_PREFIX + keyText_(key).replace(/[^a-z0-9]+/g,'_');
}

function v4179IsPdSubject_(subject) {
  const s = normalizeText_(subject);
  if (!s) return false;
  return /(^|\s)(?:PĐ|PD|P\.?Đ\.?|PHỤ\s*ĐẠO|PHU\s*DAO)(?=\s|$)/i.test(s);
}

function v4179PdBaseSubject_(subject) {
  let s = normalizeText_(subject);
  s = s.replace(/(^|\s)(?:PĐ|PD|P\.?Đ\.?|PHỤ\s*ĐẠO|PHU\s*DAO)(?=\s|$)/ig,' ');
  s = normalizeText_(s.replace(/\s+/g,' '));
  const canonical = v4157CanonicalBaseSubject_(s);
  return canonical || s;
}

function getPdRegularToggle(teacherKey) {
  const key = resolveTeacherKey_(teacherKey);
  const enabled = (PropertiesService.getScriptProperties().getProperty(v4179PdPropKey_(key)) || '') === '1';
  const meta = v4161TeacherMeta_(key);
  return {
    ok:true,
    teacherKey:key,
    teacher:meta.fullName,
    subject:meta.subject || '',
    enabled:enabled,
    label:enabled ? 'Đang bật' : 'Đang tắt',
    description:enabled ? 'Tiết PĐ đang được tính như chính khóa và lấy CT tiếp theo.' : 'Tiết PĐ đang được bỏ qua, không chiếm CT chính khóa.'
  };
}

function setPdRegularToggle(teacherKey, enabled) {
  const key = resolveTeacherKey_(teacherKey);
  const props = PropertiesService.getScriptProperties();
  const propKey = v4179PdPropKey_(key);
  if (enabled === true || String(enabled).toLowerCase() === 'true' || String(enabled) === '1') props.setProperty(propKey,'1');
  else props.deleteProperty(propKey);
  try { CacheService.getScriptCache().remove('v4179_pd_' + key); } catch(e) {}
  return getPdRegularToggle(key);
}

// Bọc bộ đọc TKB hiện hành thay vì sửa sâu các luồng PPCT cũ.
// Nhờ vậy mọi chức năng dùng TKB (xem trước, ghi báo giảng, gợi ý PPCT, lịch ngày/tuần)
// đều nhận cùng một quy tắc PĐ theo giáo viên.
const v4179ReadMathScheduleBase_ = readMathSchedule_;
readMathSchedule_ = function(teacherKey, tkbSheetName) {
  const rows = v4179ReadMathScheduleBase_(teacherKey, tkbSheetName) || [];
  const cfg = getPdRegularToggle(teacherKey);
  const out = [];
  rows.forEach(function(rec){
    if (!v4179IsPdSubject_(rec && rec.subject)) {
      out.push(rec);
      return;
    }
    // Tắt: PĐ không lên báo giảng và không làm tăng CT.
    if (!cfg.enabled) return;
    // Bật: đổi PĐ thành đúng môn chính khóa trước khi toàn bộ pipeline PPCT chạy.
    const base = v4179PdBaseSubject_(rec.subject);
    if (!base) return;
...
function V4179_KIEM_TRA_PD_GIAO_VIEN(teacherRef, tkbSheetName) {
  const key = resolveTeacherKey_(teacherRef || FIXED_TEACHER_KEY);
  const cfg = getPdRegularToggle(key);
  const raw = v4179ReadMathScheduleBase_(key, tkbSheetName) || [];
  const pd = raw.filter(r => v4179IsPdSubject_(r.subject));
  const effective = readMathSchedule_(key, tkbSheetName);
  return {ok:true,config:cfg,pdInTkb:pd.length,effectiveLessons:effective.length,pdRows:pd.map(r=>({dayNum:r.dayNum,session:r.session,period:r.period,className:r.className,subject:r.subject}))};
}


// V4.180 — công cụ kiểm tra nhanh tên giáo viên trên sheet Báo giảng.
function V4180_KIEM_TRA_KHOI_GIAO_VIEN(teacherRef, reportSheetName){
  const key=resolveTeacherKey_(teacherRef);
  const fullName=(TEACHER_PROFILES[key]||{}).fullName||TEACHER_MAP[key]||teacherRef;
  const ss=SpreadsheetApp.openById(BAO_GIANG_SPREADSHEET_ID);
  const sh=ss.getSheetByName(reportSheetName||DEFAULT_BAO_GIANG_SHEET_NAME);
  if(!sh) throw new Error('Không tìm thấy sheet Báo giảng: '+reportSheetName);
  const block=findTeacherBlockStart_(sh,fullName);
  return {ok:true,teacherKey:key,fullName:fullName,sheet:sh.getName(),blockStartColumn: block+1};
}

// ============================================================================
// V4.186 — PĐ GIỮ NGUYÊN KHI TẮT, CHỈ CHUYỂN SANG CHÍNH KHÓA KHI BẬT
// Chốt 06/09/2026:
// - TẮT "PĐ → Chính khóa": tiết PĐ vẫn lên Báo giảng bình thường,
//   chạy luồng Phụ đạo riêng PĐ1, PĐ2, PĐ3... và dùng nguồn PĐ riêng.
// - BẬT "PĐ → Chính khóa": tiết PĐ của TKB được đổi thành Chính khóa,
//   dùng PPCT chính khóa và lấy CT tiếp theo.
// - Chuyên đề CĐ luôn là luồng riêng, không thay đổi.
// ============================================================================

const V4186_TRACK_REGULAR = 'regular';
const V4186_TRACK_ELECTIVE = 'elective';
const V4186_TRACK_REMEDIAL = 'remedial';

// Ghi đè bộ nhận diện track để hỗ trợ đủ 3 luồng.
function v4135SubjectTrackInfo_(subject) {
  const raw = normalizeText_(subject);
  let base = raw;
  let track = V4186_TRACK_REGULAR;

  // Nhận PĐ trước CĐ để tránh nhập nhằng hậu tố.
  if (/\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i.test(base)) {
    track = V4186_TRACK_REMEDIAL;
    base = normalizeText_(base.replace(/\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i, ''));
  } else if (/\s+(?:CĐ|CD|CHUYÊN\s*ĐỀ|CHUYEN\s*DE)\s*$/i.test(base)) {
    track = V4186_TRACK_ELECTIVE;
    base = normalizeText_(base.replace(/\s+(?:CĐ|CD|CHUYÊN\s*ĐỀ|CHUYEN\s*DE)\s*$/i, ''));
  }
  ...
  if (!base) base = v4157CanonicalBaseSubject_(raw) || raw || 'Chưa xác định môn';
  const displayBase = base === 'GDKTPL' ? 'GDKT&PL' : base;
  const suffix = track === V4186_TRACK_ELECTIVE ? ' CĐ' : (track === V4186_TRACK_REMEDIAL ? ' PĐ' : '');
  const prefix = track === V4186_TRACK_ELECTIVE ? 'CĐ' : (track === V4186_TRACK_REMEDIAL ? 'PĐ' : '');

  return {
    raw:raw,
    baseSubject:base,
    track:track,
    isElective:track === V4186_TRACK_ELECTIVE,
    isRemedial:track === V4186_TRACK_REMEDIAL,
    displaySubject:displayBase + suffix,
    prefix:prefix
  };
}
...
function v4135TrackFromPpct_(value) {
  const txt = normalizeText_(value).toUpperCase().replace(/\s+/g, '');
  if (/^(?:PĐ|PD)\d+$/.test(txt)) return V4186_TRACK_REMEDIAL;
  if (/^(?:CĐ|CD)\d+$/.test(txt)) return V4186_TRACK_ELECTIVE;
  return V4186_TRACK_REGULAR;
}

function v4135PpctNumber_(value) {
  if (typeof value === 'number') return (Number.isFinite(value) && value > 0) ? Math.floor(value) : null;
  const txt = normalizeText_(value).toUpperCase().replace(/\s+/g, '');
  const m = txt.match(/^(?:(?:CĐ|CD|PĐ|PD))?(\d{1,3})$/);
  if (!m) return null;
  const n = Number(m[1]);
  return Number.isFinite(n) && n > 0 ? n : null;
}
...
function v4135PpctLabel_(n, track) {
  ...
  if (!Number.isFinite(n) || n < 1) return '';
  n = Math.floor(n);
  if (track === V4186_TRACK_ELECTIVE) return 'CĐ' + n;
  if (track === V4186_TRACK_REMEDIAL) return 'PĐ' + n;
  return n;
}
...
function v4127PairKey_(className, subject, track) {
  ...
  const clsKey = keyText_(className || '').replace(/[^a-z0-9]+/g, '');
  const baseKey = v4127SubjectKey_(info.baseSubject);
  const suffix = actualTrack === V4186_TRACK_ELECTIVE ? '||cd' : (actualTrack === V4186_TRACK_REMEDIAL ? '||pd' : '');
  return clsKey + '||' + baseKey + suffix;
}

// PPCT PĐ Toán 10/11/12 đã chốt trước đó: 18 tiết/lớp.
const V4186_MATH_REMEDIAL_MAP = {
  '10': {
    '1':'Mệnh đề','2':'Tập hợp và các phép toán trên tập hợp','3':'Bất phương trình bậc nhất hai ẩn','4':'Hệ bất phương trình bậc nhất hai ẩn','5':'Giá trị lượng giác của một góc từ 0° đến 180°','6':'Hệ thức lượng trong tam giác','7':'Tổng, hiệu và tích của vectơ với một số','8':'Vectơ trong mặt phẳng tọa độ và tích vô hướng','9':'Ôn tập tổng hợp học kì I','10':'Hàm số','11':'Hàm số bậc hai','12':'Dấu của tam thức bậc hai','13':'Phương trình quy về phương trình bậc hai','14':'Phương trình đường thẳng','15':'Vị trí tương đối, góc và khoảng cách giữa hai đường thẳng','16':'Đường tròn và ba đường conic','17':'Quy tắc đếm, hoán vị, chỉnh hợp, tổ hợp và nhị thức Newton','18':'Biến cố và xác suất theo định nghĩa cổ điển'
  },
  '11': {
    '1':'Giá trị lượng giác của một góc lượng giác','2':'Công thức lượng giác','3':'Hàm số lượng giác','4':'Phương trình lượng giác cơ bản','5':'Dãy số','6':'Cấp số cộng và cấp số nhân','7':'Các số đặc trưng đo xu thế trung tâm cho mẫu số liệu ghép nhóm','8':'Quan hệ song song trong không gian','9':'Giới hạn và hàm số liên tục','10':'Lũy thừa với số mũ thực','11':'Lôgarit','12':'Hàm số mũ và hàm số lôgarit','13':'Phương trình, bất phương trình mũ và lôgarit','14':'Đường thẳng vuông góc với mặt phẳng và hai mặt phẳng vuông góc','15':'Góc, khoảng cách trong không gian','16':'Thể tích','17':'Xác suất: biến cố hợp, giao; công thức cộng và nhân','18':'Đạo hàm và các quy tắc tính đạo hàm'
  },
  '12': {
    '1':'Tính đơn điệu của hàm số','2':'Cực trị của hàm số','3':'Giá trị lớn nhất, giá trị nhỏ nhất của hàm số','4':'Đường tiệm cận của đồ thị hàm số','5':'Khảo sát và vẽ đồ thị một số hàm số cơ bản','6':'Ứng dụng đạo hàm vào bài toán thực tiễn','7':'Vectơ và các phép toán trong không gian','8':'Hệ tọa độ và biểu thức tọa độ trong không gian','9':'Các số đặc trưng đo mức độ phân tán của mẫu số liệu ghép nhóm','10':'Nguyên hàm','11':'Tích phân','12':'Ứng dụng hình học của tích phân','13':'Phương trình mặt phẳng','14':'Phương trình đường thẳng trong không gian','15':'Công thức tính góc trong không gian','16':'Phương trình mặt cầu','17':'Xác suất có điều kiện','18':'Công thức xác suất toàn phần và công thức Bayes'
  }
};

(function v4186InstallMathRemedialWarehouse_(){
  try {
    const math = V4151_PPCT_WAREHOUSE && V4151_PPCT_WAREHOUSE['Toán'];
    if (!math) return;
    math.remedialMap = V4186_MATH_REMEDIAL_MAP;
    math.remedialGradeCounts = {'10':18,'11':18,'12':18};
    math.remedialEntries = 54;
    math.entries = Number(math.regularEntries || 0) + Number(math.electiveEntries || 0) + 54;
    math.titleRemedial = 'Kế hoạch PPCT phụ đạo Toán 10, 11, 12 – 18 tiết/lớp';
  } catch (e) {}
})();

// Kho chuẩn trả thêm remedialMap nếu môn có PĐ tích hợp.
function v4151WarehouseCurriculum_(subject) {
  const canonical = v4151CanonicalSubject_(subject);
  const src = v4151WarehouseSource_(canonical);
  if (!src) return null;
  return {
    map:src.map || {}, electiveMap:src.electiveMap || {}, remedialMap:src.remedialMap || {},
    configured:true, entries:Number(src.entries||0), regularEntries:Number(src.regularEntries||0),
    electiveEntries:Number(src.electiveEntries||0), remedialEntries:Number(src.remedialEntries||0),
    kind:'Kho PPCT chuẩn', title:'Nguồn chuẩn của trường · ' + canonical,
    error:'', subject:canonical, gradeCounts:src.gradeCounts || {},
    electiveGradeCounts:src.electiveGradeCounts || {}, remedialGradeCounts:src.remedialGradeCounts || {},
    format:src.format || 'Tích hợp sẵn từ Kế hoạch dạy học 2026–2027',
    warnings:src.warnings || [], warningCount:Number(src.warningCount||0),
    criticalWarnings:src.criticalWarnings||[], criticalWarningCount:Number(src.criticalWarningCount||0),
    partial:!!src.partial, sourceType:'school', isOverride:false, defaultAvailable:true,
    sourceFile:src.title || '', warehouseVersion:'2026-09-06'
  };
}

const v4186LoadPpctCurriculumBase_ = loadPpctCurriculum_;
loadPpctCurriculum_ = function(subject) {
  const canonical = v4151CanonicalSubject_(subject);
  const data = v4186LoadPpctCurriculumBase_(canonical) || {};
  const school = v4151WarehouseCurriculum_(canonical);
  if (!data.remedialMap && school && school.remedialMap) data.remedialMap = school.remedialMap;
  if (!data.remedialGradeCounts && school && school.remedialGradeCounts) data.remedialGradeCounts = school.remedialGradeCounts;
  if (!data.remedialEntries && school && school.remedialEntries) data.remedialEntries = school.remedialEntries;
  return data;
};
...
function externalLessonFor_(curriculum, grade, ppct, track) {
  ...
  const n = v4135PpctNumber_(ppct);
  if (!n) return '';
  let root = curriculum && curriculum.map;
  if (actualTrack === V4186_TRACK_ELECTIVE) root = curriculum && curriculum.electiveMap;
  else if (actualTrack === V4186_TRACK_REMEDIAL) root = curriculum && curriculum.remedialMap;
  const byGrade = root && root[String(grade)];
  return byGrade && byGrade[String(n)] ? normalizeText_(byGrade[String(n)]) : '';
}

function getPpctSourceConfig(subject) {
  const canonical = v4151CanonicalSubject_(normalizeText_(subject) || getTeacherDirectoryInfo_(FIXED_TEACHER_KEY).subject || 'Toán');
  const data = loadPpctCurriculum_(canonical) || {};
  const school = v4151WarehouseSource_(canonical);
  const url = getPpctSourceUrl_(canonical);
  return {
    subject:canonical, url:url, configured:!!(url || school), defaultAvailable:!!school, isOverride:!!url,
    entries:Number(data.entries||0), regularEntries:Number(data.regularEntries||0), electiveEntries:Number(data.electiveEntries||0), remedialEntries:Number(data.remedialEntries||0),
    kind:data.kind || (school ? 'Kho PPCT chuẩn' : ''), title:data.title || (school ? ('Nguồn chuẩn của trường · '+canonical) : ''), error:data.error || '',
    gradeCounts:data.gradeCounts || {}, electiveGradeCounts:data.electiveGradeCounts || {}, remedialGradeCounts:data.remedialGradeCounts || {},
    format:data.format || '', warnings:data.warnings || [], warningCount:Number(data.warningCount||0), warehouseVersion:'2026-09-06',
    tracks:{regular:true,elective:Number(data.electiveEntries||0)>0,remedial:Number(data.remedialEntries||0)>0}
  };
}

// Tiến độ 3 khu: Chính khóa / Chuyên đề / Phụ đạo đại trà.
function v4158ProgressColumnMap_(sheet) {
  const lastCol = Math.min(Math.max(sheet.getLastColumn(), 27), 80);
  const rows = sheet.getRange(1, 1, 2, lastCol).getDisplayValues();
  const groupRow = rows[0] || [], subjectRow = rows[1] || [];
  const regular = {}, elective = {}, remedial = {};
  let currentGroup = '';
  function groupType_(value) {
    const k = keyText_(value || '').replace(/[^a-z0-9]+/g, '');
    if (!k) return '';
    if (k.indexOf('chuyende') >= 0) return 'elective';
    if (k.indexOf('phudaodaitra') >= 0) return 'remedial';
    if (k.indexOf('monhdgdbatbuoc') >= 0 || k.indexOf('monhocluachon') >= 0) return 'regular';
    return 'other';
  }
  for (let c = 1; c <= lastCol; c++) {
    const groupLabel = normalizeText_(groupRow[c - 1]);
    if (groupLabel) currentGroup = groupType_(groupLabel);
    if (c < 4) continue;
    const base = v4158ProgressHeaderSubject_(subjectRow[c - 1]);
    if (!base) continue;
    const sk = v4127SubjectKey_(base);
    if (!sk) continue;
    if (currentGroup === 'elective') { if (elective[sk] == null) elective[sk] = c; }
    else if (currentGroup === 'remedial') { if (remedial[sk] == null) remedial[sk] = c; }
    else if (currentGroup === 'regular') { if (regular[sk] == null) regular[sk] = c; }
  }
  return {regular:regular,elective:elective,remedial:remedial};
}
...
function v4158ProgressPairsFromRecords_(records) {
  ...
  (records || []).forEach(r => {
    ...
    if (!base) return;
    const ppctTrack = v4135TrackFromPpct_(r && r.ppct);
    let track = (r && r.track) || info.track || V4186_TRACK_REGULAR;
    if (ppctTrack !== V4186_TRACK_REGULAR) track = ppctTrack;
    if (track !== V4186_TRACK_ELECTIVE && track !== V4186_TRACK_REMEDIAL) track = V4186_TRACK_REGULAR;
    const n = Number(r && r.ppctNumber) > 0 ? Math.floor(Number(r.ppctNumber)) : v4135PpctNumber_(r && r.ppct);
    if (!n) return;
    const key = cls.toLowerCase() + '|' + v4127SubjectKey_(base) + '|' + track;
    if (!pairs[key] || n > pairs[key].ppct) pairs[key] = {key:key,className:cls,baseSubject:base,subjectKey:v4127SubjectKey_(base),track:track,ppct:n};
  });
  return Object.keys(pairs).map(k => pairs[k]);
}

function v4186ProgressMapForTrack_(colMap, track) {
  if (track === V4186_TRACK_ELECTIVE) return colMap.elective || {};
  if (track === V4186_TRACK_REMEDIAL) return colMap.remedial || {};
  return colMap.regular || {};
}

function v4186ProgressTrackLabel_(track) {
  if (track === V4186_TRACK_ELECTIVE) return 'Chuyên đề';
  if (track === V4186_TRACK_REMEDIAL) return 'Phụ đạo đại trà';
  return 'Chính khóa';
}
...
function updateProgressReport_(payload, records) {
  ...
  const pairs = v4158ProgressPairsFromRecords_(records);
  if (!pairs.length) return [];
  const ss = SpreadsheetApp.openById(TIEN_DO_SPREADSHEET_ID), groups = {};
  pairs.forEach(p => { const sn = progressSheetNameForClass_(p.className); (groups[sn] || (groups[sn] = [])).push(p); });
  const details = [];
  Object.keys(groups).forEach(sheetName => {
    const sh = ss.getSheetByName(sheetName);
    if (!sh) throw new Error('Không tìm thấy sheet tiến độ: ' + sheetName);
    const rowMap = v4158ProgressRowMap_(sh, week), colMap = v4158ProgressColumnMap_(sh);
    groups[sheetName].forEach(p => {
      const targetRow = rowMap[p.className.toLowerCase()];
      if (!targetRow) throw new Error('Không tìm thấy dòng Tuần ' + week + ' - ' + p.className + ' trong ' + sheetName);
      const targetCol = v4186ProgressMapForTrack_(colMap, p.track)[p.subjectKey];
      if (!targetCol) throw new Error('Không tìm thấy cột ' + p.baseSubject + ' trong khu ' + v4186ProgressTrackLabel_(p.track) + ' của sheet ' + sheetName + '.');
      const range = sh.getRange(targetRow, targetCol); range.setValue(p.ppct);
      details.push({sheet:sheetName,className:p.className,subject:p.baseSubject,track:p.track,trackLabel:v4186ProgressTrackLabel_(p.track),week:week,ppct:p.ppct,range:range.getA1Notation()});
    });
  });
  return details;
}

// Cập nhật ý nghĩa nút: TẮT = PĐ bình thường, BẬT = đổi PĐ sang chính khóa.
function getPdRegularToggle(teacherKey) {
  const key = resolveTeacherKey_(teacherKey);
  const enabled = (PropertiesService.getScriptProperties().getProperty(v4179PdPropKey_(key)) || '') === '1';
  const meta = v4161TeacherMeta_(key);
  return {
    ok:true, teacherKey:key, teacher:meta.fullName, subject:meta.subject || '', enabled:enabled,
    label:enabled ? 'Đang bật' : 'Đang tắt',
    description:enabled
      ? 'Tuần này: tiết PĐ được chuyển sang Chính khóa, dùng PPCT chính khóa và lấy CT tiếp theo.'
      : 'Tiết PĐ vẫn lên bình thường theo luồng PĐ riêng (PĐ1, PĐ2, PĐ3...).'
  };
}

// Đây là điểm quyết định duy nhất của công tắc.
// Dùng raw schedule gốc để tránh lớp wrapper V4.179 cũ bỏ mất PĐ khi nút tắt.
readMathSchedule_ = function(teacherKey, tkbSheetName) {
  const rows = v4179ReadMathScheduleBase_(teacherKey, tkbSheetName) || [];
  const cfg = getPdRegularToggle(teacherKey);
  return rows.map(function(rec){
    if (!v4179IsPdSubject_(rec && rec.subject)) return rec;

    // TẮT: giữ nguyên PĐ => pipeline 3 luồng sẽ nhận track=remedial.
    if (!cfg.enabled) {
      const pdBase = v4179PdBaseSubject_(rec.subject);
      return Object.assign({}, rec, {
        subject:(pdBase ? (pdBase + ' PĐ') : rec.subject),
        baseSubject:pdBase || rec.subject,
        track:V4186_TRACK_REMEDIAL,
        isElective:false,
        isRemedial:true,
        pdConverted:false,
    ...
    // BẬT: đổi PĐ thành Chính khóa => dùng chung bộ đếm CT chính khóa.
    const base = v4179PdBaseSubject_(rec.subject);
    return Object.assign({}, rec, {
      subject:base,
      baseSubject:base,
  ...
  }).filter(r => !!normalizeText_(r && r.subject));
};

function V4186_KIEM_TRA_LOGIC_PD(teacherRef, tkbSheetName) {
  const key = resolveTeacherKey_(teacherRef || FIXED_TEACHER_KEY);
  const cfg = getPdRegularToggle(key);
  const raw = v4179ReadMathScheduleBase_(key, tkbSheetName) || [];
  const effective = readMathSchedule_(key, tkbSheetName) || [];
  const pdRaw = raw.filter(r => v4179IsPdSubject_(r && r.subject));
  const pdEffective = effective.filter(r => (r && (r.isRemedial || r.pdConverted)));
  return {
    ok:true, version:'4.186', config:cfg, pdInTkb:pdRaw.length,
    behavior:cfg.enabled ? 'PĐ → Chính khóa' : 'PĐ giữ nguyên luồng PĐ',
    sample:pdEffective.map(r => ({dayNum:r.dayNum,session:r.session,period:r.period,className:r.className,subject:r.subject,track:r.track,pdConverted:!!r.pdConverted})).slice(0,20)
  };
}


// ============================================================================
// V4.187 — KIỂM TRA TIẾN ĐỘ LIỀN MẠCH TOÀN BỘ BÁO GIẢNG
// Quét các tuần đã có trong Lịch báo giảng theo từng Giáo viên → Môn → Lớp → Luồng.
// Phát hiện: lặp tiết, nhảy cóc, đi lùi. Chính khóa / CĐ / PĐ được kiểm tra độc lập.
// ============================================================================
function v4187TrackLabel_(track) {
  if (track === V4186_TRACK_ELECTIVE) return 'Chuyên đề';
  if (track === V4186_TRACK_REMEDIAL) return 'Phụ đạo';
  return 'Chính khóa';
}

function v4187PpctLabel_(n, track) {
  if (track === V4186_TRACK_ELECTIVE) return 'CĐ' + n;
  if (track === V4186_TRACK_REMEDIAL) return 'PĐ' + n;
  return 'CT' + n;
}

function v4187ReadTeacherWeekRecords_(sheet, fullName, weekMeta) {
  const out = [];
  let blockStartZero;
  try { blockStartZero = findTeacherBlockStart_(sheet, fullName); }
  catch (e) { return out; }

  let sections;
  try { sections = findTeacherSectionRows_(sheet, blockStartZero); }
  catch (e) { return out; }

  const startCol1 = blockStartZero + 1;
  const sectionDefs = [
    {name:'Sáng', titleRow:sections.morning, order:0},
    {name:'Chiều', titleRow:sections.afternoon, order:1}
  ];

  sectionDefs.forEach(function(sec){
    if (!sec.titleRow) return;
    const firstDataRow = sec.titleRow + 3;
    const vals = sheet.getRange(firstDataRow, startCol1, 30, 7).getDisplayValues();
    for (let i=0;i<vals.length;i++) {
      const row=vals[i]||[];
      const subjectRaw=normalizeText_(row[2]);
      const className=normalizeText_(row[3]).replace(/\s+/g,'');
      const ppctRaw=normalizeText_(row[4]);
      if (!subjectRaw || !className || !ppctRaw) continue;
      if (!/^(10|11|12)[A-Za-zÀ-ỹ0-9._-]+$/i.test(className)) continue;

      const ppctNum=v4135PpctNumber_(ppctRaw);
      if (!(ppctNum>0)) continue;
      const subjectInfo=v4135SubjectTrackInfo_(subjectRaw);
      const ppctTrack=v4135TrackFromPpct_(ppctRaw);
      const track=(ppctTrack!==V4186_TRACK_REGULAR) ? ppctTrack : subjectInfo.track;
      const baseSubject=subjectInfo.baseSubject;
      if (!baseSubject || baseSubject==='Chưa xác định môn') continue;

      const dayNum=2+Math.floor(i/5);
      const period=(i%5)+1;
      out.push({
        sheet:sheet.getName(),
        weekToken:weekMeta.token||'',
        monday:weekMeta.monday||'',
        weekLabel:weekMeta.label||('Tuần '+compactWeekLabel_(weekMeta.token||'')),
        session:sec.name,
        sessionOrder:sec.order,
        dayNum:dayNum,
        period:period,
        subject:baseSubject,
        subjectDisplay:baseSubject==='GDKTPL'?'GDKT&PL':baseSubject,
        className:className,
        track:track,
        ppct:ppctNum,
        ppctLabel:v4187PpctLabel_(ppctNum,track),
        lesson:normalizeText_(row[5])
      });
    }
  });
...
function auditProgressContinuity(payload) {
  payload=payload||{};
  const teacherKey=resolveTeacherKey_(payload.teacherKey||payload.teacherRef||'');
  const fullName=(TEACHER_PROFILES[teacherKey]&&TEACHER_PROFILES[teacherKey].fullName)||TEACHER_MAP[teacherKey]||teacherKey;
  if (!fullName) throw new Error('Không xác định được giáo viên.');

  const ss=SpreadsheetApp.openById(BAO_GIANG_SPREADSHEET_ID);
  const weeks=listWeekSheets_().slice().sort(function(a,b){
    const da=parseYmd_(a.monday), db=parseYmd_(b.monday);
    return (da?da.getTime():0)-(db?db.getTime():0);
  });
  const records=[];
  weeks.forEach(function(w){
    const sh=ss.getSheetByName(w.name);
    if (!sh) return;
    Array.prototype.push.apply(records,v4187ReadTeacherWeekRecords_(sh,fullName,w));
  });
  ...
  records.forEach(function(r){
    const key=[keyText_(r.subject),keyText_(r.className),r.track].join('||');
    if(!groups[key]) groups[key]={subject:r.subjectDisplay,className:r.className,track:r.track,items:[]};
  ...
  Object.keys(groups).sort().forEach(function(k){
    const g=groups[k], items=g.items, issues=[];
    for(let i=1;i<items.length;i++){
      const prev=items[i-1], cur=items[i];
      if(cur.ppct===prev.ppct){
        issues.push({type:'duplicate',severity:'error',message:'Lặp '+cur.ppctLabel,previous:prev,current:cur,expected:v4187PpctLabel_(prev.ppct+1,g.track)});
      } else if(cur.ppct>prev.ppct+1){
        issues.push({type:'gap',severity:'error',message:'Thiếu '+v4187PpctLabel_(prev.ppct+1,g.track)+' → '+v4187PpctLabel_(cur.ppct-1,g.track),previous:prev,current:cur,expected:v4187PpctLabel_(prev.ppct+1,g.track)});
      } else if(cur.ppct<prev.ppct){
        issues.push({type:'backward',severity:'error',message:'Đi lùi '+prev.ppctLabel+' → '+cur.ppctLabel,previous:prev,current:cur,expected:v4187PpctLabel_(prev.ppct+1,g.track)});
      }
    }
    ...
    sequences.push({
      ...
      track:g.track,
      trackLabel:v4187TrackLabel_(g.track),
      count:items.length,
      first:first?first.ppctLabel:'',
      last:last?last.ppctLabel:'',
      nextExpected:last?v4187PpctLabel_(last.ppct+1,g.track):'',
  ...
  return {
    ...
    teacherKey:teacherKey,
    teacher:fullName,
    checkedWeeks:weeks.map(function(w){return w.name;}),
    recordCount:records.length,
    sequenceCount:sequences.length,
    issueCount:issueCount,
    overallStatus:issueCount?'error':'ok',
    sequences:sequences
  };
}

// ============================================================================
// V4.192 — SINH HỌC VỀ KHO PPCT CHUẨN + PĐ ANH / SINH / NGỮ VĂN TỪ WORD
// - Sinh học chính khóa dùng Kho PPCT chuẩn của trường: 70 tiết/khối 10, 11, 12.
// - PĐ Tiếng Anh: 18 tiết/khối 10, 11, 12.
// - PĐ Sinh học: 12 tiết khối 10 và 12 (file Word không có khối 11).
// - PĐ Ngữ văn: 21 tiết khối 10 (file Word hiện có chỉ khối 10).
// - PĐ chạy luồng riêng PĐ1, PĐ2...; không trộn Chính khóa và Chuyên đề.
// ============================================================================
const V4192_PPCT_WAREHOUSE_VERSION = '2026-09-13-PD-ANH-SINH-VAN';
const V4192_REMEDIAL_MAPS = {"Tiếng Anh":{"12":{"1":"Tense revision","2":"Tense revision","3":"Articles","4":"Verbs with prepositions","5":"Relative clauses","6":"Double comparatives","7":"Simple, compound, and complex sentences","8":"Reading skills","9":"Reading skills","10":"Active and passive causatives","11":"Adverbial clauses","12":"Adverbial clauses","13":"Phrasal verbs","14":"Phrasal verbs","15":"Reported speech","16":"Reported speech","17":"Practice exercises","18":"Practice exercises"},"11":{"1":"Tense revision","2":"Tense revision","3":"Modal verbs","4":"Modal verbs","5":"Stative verbs in the continuous form","6":"Gerunds","7":"Present and past participle clauses","8":"Reading","9":"Practice exercises","10":"To- infinitive , Perfect gerunds and perfect participle clauses","11":"Cleft sentences","12":"Rewriting sentences","13":"Rewriting sentences","14":"Linking words and phrases","15":"Linking words and phrases","16":"Compound nouns","17":"Word forms","18":"Practice exercises"},"10":{"1":"Review of Tenses","2":"Review of Tenses","3":"Passive voice","4":"Passive voice","5":"Compound sentences","6":"Gerunds and infinitives","7":"Word forms","8":"Reading skill","9":"Practice exercises","10":"Comparatives and superlatives","11":"Stress","12":"Relative clauses","13":"Relative clauses","14":"Reported speech","15":"Reported speech","16":"Conditional sentences","17":"Conditional sentences","18":"Practice exercises"}},"Sinh học":{"10":{"1":"Các phân tử sinh học","2":"Các phân tử sinh học","3":"- Trả chữa bài kiểm tra giữa kì I. - Cấu trúc tế bào.","4":"- Trả chữa bài kiểm tra giữa kì I. - Cấu trúc tế bào.","5":"Trả, chữa bài kiểm tra cuối kì I","6":"Chuyển hóa năng lượng trong tế bào","7":"Nguyên phân và giảm phân","8":"Nguyên phân và giảm phân","9":"Vi sinh vật","10":"Virus","11":"Trả, chữa bài kiểm tra giữa kì II","12":"Ôn tập cuối kì II"},"12":{"1":"Di truyền phân tử","2":"Di truyền phân tử","3":"Học thuyết Mendel","4":"Mở rộng học thuyết Mendel và Di truyền giới tính và di truyền liên kết với giới tính","5":"Liên kết gene và hoán vị gene","6":"Trả chữa bài kiểm tra giữa kì I","7":"Đột biến nhiễm sắc thể","8":"Di truyền quần thể","9":"Trả chữa bài kiểm tra cuối kì I","10":"Tiến hóa","11":"Trả bài kiểm tra giữa kì II + Quần xã","12":"Hệ sinh thái"}},"Ngữ văn":{"10":{"1":"Viết: Viết văn bản nghị luận phân tích, đánh giá một tác phẩm truyện (Chủ đề, những nét đặc sắc về hình thức nghệ thuật)","2":"Nói và nghe: Rèn kỹ năng nói và nghe","3":"Viết: Ôn tập cách viết văn bản nghị luận phân tích, đánh giá một tác phẩm thơ","4":"Nói và nghe: Rèn kỹ năng nói và nghe","5":"Viết: Ôn tập viết bài luận thuyết phục người khác từ bỏ một thói quen hay một quan niệm","6":"Đọc: Rèn kỹ năng đọc văn bản","7":"Viết: Ôn tập viết báo cáo nghiên cứu về một vấn đề","8":"Luyện đề tổng hợp","9":"Ôn tập kiểm tra cuối HKI","10":"Ôn tập kiểm tra cuối HKI","11":"Đọc: Rèn kỹ năng đọc văn bản","12":"Viết: Ôn tập viết bài văn nghị luận phân tích, đánh giá một tác phẩm văn học (Chủ đề và nhân vật trong tác phẩm truyện)","13":"Đọc: Rèn kỹ năng đọc văn bản","14":"Nói và nghe: Rèn kỹ năng nói và nghe","15":"Luyện đề tổng hợp + ôn tập kiểm tra giữa HKII","16":"Viết: Ôn tập viết một văn bản nội quy hoặc văn bản hướng dẫn nơi công cộng","17":"Nói và nghe: Rèn kỹ năng nói và nghe","18":"Viết: Ôn tập viết bài luận về bản thân","19":"Luyện đề tổng hợp","20":"Luyện đề + Ôn tập kiểm tra cuối HKII","21":"Luyện đề + Ôn tập kiểm tra cuối HKII"}}};

(function v4192InstallSchoolCurricula_(){
  try {
    // Chốt Sinh học chính khóa: đủ đúng 70 tiết cho mỗi khối.
    const biology = V4151_PPCT_WAREHOUSE && V4151_PPCT_WAREHOUSE['Sinh học'];
    if (!biology || !biology.map) throw new Error('Không tìm thấy Sinh học trong Kho PPCT chuẩn.');
    ['10','11','12'].forEach(function(g){
      const count = Object.keys((biology.map && biology.map[g]) || {}).length;
      if (count !== 70) throw new Error('Kho Sinh học ' + g + ' phải đủ 70 tiết, hiện có ' + count + '.');
    });
    biology.gradeCounts = {'10':70,'11':70,'12':70};
    biology.regularEntries = 210;

    Object.keys(V4192_REMEDIAL_MAPS).forEach(function(subject){
      const src = V4151_PPCT_WAREHOUSE && V4151_PPCT_WAREHOUSE[subject];
      if (!src) return;
      const map = V4192_REMEDIAL_MAPS[subject] || {};
      const counts = {};
      let remedialEntries = 0;
      Object.keys(map).forEach(function(grade){
        counts[grade] = Object.keys(map[grade] || {}).length;
        remedialEntries += counts[grade];
      });
      src.remedialMap = map;
      src.remedialGradeCounts = counts;
      src.remedialEntries = remedialEntries;
      src.entries = Number(src.regularEntries || 0) + Number(src.electiveEntries || 0) + remedialEntries;
      src.titleRemedial = subject === 'Tiếng Anh'
        ? 'Kế hoạch phụ đạo Tiếng Anh 10, 11, 12 từ file Word'
        : (subject === 'Sinh học'
          ? 'Kế hoạch dạy phụ đạo Sinh học 10, 12 năm học 2026–2027'
          : 'Kế hoạch phụ đạo Ngữ văn 10 từ file Word');
    });
  } catch (e) {
    console.error('V4.192 cài Kho PPCT:', e);
    throw e;
  }
})();

// Sinh học được đưa hẳn về Kho PPCT chuẩn của trường, không để link cá nhân cũ
// vô tình ghi đè nội dung chuẩn. Các môn khác vẫn giữ cơ chế Thay nguồn như cũ.
const v4192GetPpctSourceUrlBase_ = getPpctSourceUrl_;
getPpctSourceUrl_ = function(subject) {
  const canonical = v4151CanonicalSubject_(subject || '');
  if (canonical === 'Sinh học') return '';
  return v4192GetPpctSourceUrlBase_(subject);
};

// Đồng bộ version + thông tin nguồn sau khi V4.192 được cài.
const v4192GetPpctSourceConfigBase_ = getPpctSourceConfig;
getPpctSourceConfig = function(subject) {
  const cfg = v4192GetPpctSourceConfigBase_(subject) || {};
  cfg.warehouseVersion = V4192_PPCT_WAREHOUSE_VERSION;
  if (cfg.subject === 'Sinh học') {
    cfg.url = '';
    cfg.isOverride = false;
    cfg.defaultAvailable = true;
    cfg.kind = 'Kho PPCT chuẩn';
    cfg.title = 'Nguồn chuẩn của trường · Sinh học';
  }
  return cfg;
};
...
// ============================================================================
// V4.193 — SỬA CHUẨN BÁO CÁO TIẾN ĐỘ: GHI SỐ TIẾT THỰC DẠY TRONG TUẦN
// - Bảng "Báo cáo tiến độ thực hiện chương trình" cần số tiết của TỪNG TUẦN,
//   không phải số PPCT lớn nhất đã chạy tới.
// - Đếm độc lập theo Lớp + Môn + Luồng: Chính khóa / Chuyên đề / Phụ đạo.
// - PPCT (CT/CĐ/PĐ) vẫn dùng để xác định bài dạy và kiểm tra liên mạch; không
//   được ghi trực tiếp số PPCT vào bảng tổng hợp tuần.
// ============================================================================
const V4193_VERSION = '4.193';

function v4193ProgressCountsFromRecords_(records) {
  const pairs = {};
  (records || []).forEach(function(r) {
    const cls = normalizeText_(r && r.className).replace(/\s+/g, '');
    if (!v4146ClassGrade_(cls)) return;
    ...
    // Chỉ một dòng báo giảng có PPCT hợp lệ mới được tính là một tiết thực dạy.
    const n = Number(r && r.ppctNumber) > 0 ? Math.floor(Number(r.ppctNumber)) : v4135PpctNumber_(r && r.ppct);
    if (!n) return;

    const subjectKey = v4127SubjectKey_(base);
    const key = cls.toLowerCase() + '|' + subjectKey + '|' + track;
    if (!pairs[key]) {
      pairs[key] = {
        key:key,
        className:cls,
        ...
        firstPpct:n,
        lastPpct:n,
        ppct:n // giữ tương thích với snapshot/đoạn mã cũ, KHÔNG dùng làm giá trị ghi
      };
    }
    pairs[key].count += 1;
    pairs[key].firstPpct = Math.min(pairs[key].firstPpct, n);
    pairs[key].lastPpct = Math.max(pairs[key].lastPpct, n);
    pairs[key].ppct = pairs[key].lastPpct;
  });
  return Object.keys(pairs).map(function(k){ return pairs[k]; });
}

// Ghi đè hàm gom tiến độ cũ để mọi nơi (snapshot/verify/update) cùng dùng số tiết.
function v4158ProgressPairsFromRecords_(records) {
  return v4193ProgressCountsFromRecords_(records);
}
...
function updateProgressReport_(payload, records) {
  ...
  const pairs = v4193ProgressCountsFromRecords_(records);
  if (!pairs.length) return [];
  ...
  Object.keys(groups).forEach(function(sheetName){
    ...
    groups[sheetName].forEach(function(p){
      const targetRow = rowMap[p.className.toLowerCase()];
      if (!targetRow) throw new Error('Không tìm thấy dòng Tuần ' + week + ' - ' + p.className + ' trong ' + sheetName);

      const targetCol = v4186ProgressMapForTrack_(colMap, p.track)[p.subjectKey];
      if (!targetCol) {
        throw new Error('Không tìm thấy cột ' + p.baseSubject + ' trong khu ' + v4186ProgressTrackLabel_(p.track) + ' của sheet ' + sheetName + '.');
      }

      const range = sh.getRange(targetRow, targetCol);
      range.setValue(p.count);
      details.push({
        sheet:sheetName,
        className:p.className,
        subject:p.baseSubject,
        track:p.track,
        trackLabel:v4186ProgressTrackLabel_(p.track),
        week:week,
        periods:p.count,
        firstPpct:p.firstPpct,
        lastPpct:p.lastPpct,
        range:range.getA1Notation(),
        gid:sh.getSheetId()
      });
  ...
  console.log('V4.193 updateProgressReport_: ghi số tiết thực dạy/tuần; cells=' + details.length);
  return details;
}

// Kiểm tra sau ghi cũng phải so với SỐ TIẾT, không so với PPCT cuối tuần.
function kiemTraTienDoDaCapNhat(payload) {
  try {
    const week = Number(payload && payload.week || 1);
    const records = v4158ReadTeacherProgressRecordsFromReport_(payload);
    const pairs = v4193ProgressCountsFromRecords_(records);
    if (!pairs.length) return {ok:true, done:true, pairs:0, source:'Lịch báo giảng', mode:'weekly-period-count'};
    ...
    for (const p of pairs) {
      ...
      if (!cache[sheetName]) {
        const sh = ss.getSheetByName(sheetName);
        if (!sh) return {ok:false, done:false, pairs:matched, error:'Không tìm thấy ' + sheetName};
        cache[sheetName] = {sh:sh, rows:v4158ProgressRowMap_(sh, week), cols:v4158ProgressColumnMap_(sh)};
      }
      const x = cache[sheetName];
      const row = x.rows[p.className.toLowerCase()];
      const col = v4186ProgressMapForTrack_(x.cols, p.track)[p.subjectKey];
      if (!row || !col) return {ok:true, done:false, pairs:matched, mode:'weekly-period-count'};
      const actual = Number(x.sh.getRange(row, col).getValue());
      if (actual !== Number(p.count)) return {ok:true, done:false, pairs:matched, mode:'weekly-period-count'};
      matched++;
    }
    return {ok:true, done:matched===pairs.length, pairs:matched, source:'Lịch báo giảng', mode:'weekly-period-count'};
  } catch(e) {
    return {ok:false, done:false, pairs:0, error:e && e.message ? e.message : String(e), mode:'weekly-period-count'};
  }
}

// Test logic thuần: bảo đảm không quay lại lỗi ghi PPCT cuối tuần.
function V4193_KIEM_TRA_DEM_TIET_TUAN() {
  const sample = [
    {className:'10A1',subject:'Toán',ppct:'10'},
    {className:'10A1',subject:'Toán',ppct:'11'},
    {className:'10A1',subject:'Toán',ppct:'12'},
    {className:'10A1',subject:'Toán CĐ',ppct:'CĐ4'},
    {className:'10A1',subject:'Toán PĐ',ppct:'PĐ7'},
    {className:'10A1',subject:'Toán PĐ',ppct:'PĐ8'}
  ];
  const got = v4193ProgressCountsFromRecords_(sample);
  function find_(track){ return got.filter(function(x){return x.track===track;})[0]; }
  const r=find_(V4186_TRACK_REGULAR), c=find_(V4186_TRACK_ELECTIVE), p=find_(V4186_TRACK_REMEDIAL);
  const ok=!!(r&&r.count===3&&r.lastPpct===12&&c&&c.count===1&&c.lastPpct===4&&p&&p.count===2&&p.lastPpct===8);
  return {ok:ok,version:V4193_VERSION,regular:r||null,elective:c||null,remedial:p||null};
}

// ============================================================================
// V4.196 — TÁCH GHI BÁO GIẢNG VÀ CẬP NHẬT TIẾN ĐỘ
// - UI gọi ghiBaoGiangStep1() riêng: chỉ ghi Lịch báo giảng.
// - Nút CẬP NHẬT TIẾN ĐỘ gọi capNhatTienDoStep2() riêng khi người dùng chủ động.
// - Tiến độ tiếp tục dùng cơ chế V4.193: ghi SỐ TIẾT thực dạy/tuần, tách CT/CĐ/PĐ.
// ============================================================================
const V4194_VERSION = '4.194';


// ============================================================================
// V4.200 — XEM TRƯỚC CHI TIẾT TIẾN ĐỘ THEO LỚP + CẢNH BÁO GHI ĐÈ TUẦN
// - Chọn tuần nào thì đọc/ghi đúng tuần đó.
// - Trước khi ghi, trả về số tiết mới theo từng lớp/môn/luồng và giá trị đang có.
...
function xemTruocCapNhatTienDo(payload) {
  try {
    ...
    const pairs = v4193ProgressCountsFromRecords_(records);
    if (!pairs.length) {
      return {ok:true, week:week, source:'Lịch báo giảng', details:[], classes:[], hasExisting:false, total:0, message:'Không có tiết thực dạy hợp lệ trong tuần này.'};
    }
    ...
    pairs.forEach(function(p){
      ...
      const row = x.rows[p.className.toLowerCase()];
      const col = v4186ProgressMapForTrack_(x.cols, p.track)[p.subjectKey];
      if (!row) throw new Error('Không tìm thấy dòng Tuần ' + week + ' - ' + p.className + ' trong ' + sheetName);
      if (!col) throw new Error('Không tìm thấy cột ' + p.baseSubject + ' trong khu ' + v4186ProgressTrackLabel_(p.track) + ' của sheet ' + sheetName + '.');
      const range = x.sh.getRange(row, col);
      const raw = range.getValue();
      const oldBlank = raw === '' || raw === null;
      if (!oldBlank) hasExisting = true;
      details.push({
        className:p.className, subject:p.baseSubject, track:p.track, trackLabel:v4186ProgressTrackLabel_(p.track),
        oldValue: oldBlank ? '' : raw, newValue:p.count, changed: oldBlank ? true : Number(raw) !== Number(p.count),
        sheet:sheetName, range:range.getA1Notation(), gid:x.sh.getSheetId()
      });
    });
    ...
    details.forEach(function(d){
      const k=d.className;
      if(!classMap[k]) classMap[k]={className:k,total:0,regular:0,elective:0,remedial:0,items:[],hasExisting:false,changed:false};
      const c=classMap[k];
      c.total += Number(d.newValue)||0;
      if(d.track===V4186_TRACK_ELECTIVE) c.elective += Number(d.newValue)||0;
      else if(d.track===V4186_TRACK_REMEDIAL) c.remedial += Number(d.newValue)||0;
      else c.regular += Number(d.newValue)||0;
      c.items.push(d);
      if(d.oldValue!=='' && d.oldValue!==null) c.hasExisting=true;
      if(d.changed) c.changed=true;
    });
    const classes = Object.keys(classMap).sort(function(a,b){ return a.localeCompare(b,'vi',{numeric:true}); }).map(function(k){return classMap[k];});
    const total = classes.reduce(function(s,c){return s+c.total;},0);
    return {ok:true, week:week, source:'Lịch báo giảng', details:details, classes:classes, hasExisting:hasExisting, total:total, spreadsheetId:TIEN_DO_SPREADSHEET_ID};
  } catch(e) {
    return {ok:false, error:e && e.message ? e.message : String(e), details:[], classes:[], hasExisting:false, total:0};
  }
}


// V4.197 — Cập nhật tiến độ an toàn: trả URL đúng sheet/vùng vừa ghi để UI mở trực tiếp.
