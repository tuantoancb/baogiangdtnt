...
export default function AdminPage(){
  ...
  if(!token||!data)return <main className="adminShell"><section className="adminLoginCard"><div className="adminShield">🔐</div><span className="adminEyebrow">QUẢN TRỊ GIÁO VIÊN · V4.164</span><h1>Đăng nhập quản trị</h1><p>Quản lý giáo viên đã dùng/chưa dùng, chuyển Chỉ xem hoặc khóa app ở backend.</p><form onSubmit={login}><input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Mật khẩu quản trị" autoFocus/><button disabled={busy||!password}>{busy?'Đang kiểm tra…':'Đăng nhập'}</button></form>{error&&<div className="adminError">{error}</div>}<Link href="/">← Về Cổng giáo viên</Link></section></main>;
  return <main className="adminShell"><section className="adminTop"><div><span className="adminEyebrow">QUẢN TRỊ GIÁO VIÊN · V4.164</span><h1>Quản lý sử dụng Báo giảng</h1><p>Dữ liệu quản lý lưu trong sheet <b>{data.sheetName}</b>. Khóa/Chỉ xem được kiểm tra ở backend.</p></div><div className="adminTopActions"><button onClick={()=>load()} disabled={busy}>↻ Làm mới</button><button onClick={logout}>Đăng xuất</button><Link href="/">Cổng giáo viên →</Link></div></section>
