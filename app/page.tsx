...
function TeacherCard({t,compact=false}:{t:Teacher;compact?:boolean}){
  return <Link href={`/gv/${t.slug}`} className={`teacherCard ${compact?'compact':''}`}>
    ...
    <span className="teacherInfo">
      <b>{t.fullName}</b>
      <small>{t.subject} · {t.team}{t.role?` · ${t.role}`:''}</small>
    </span>
...
function LeadershipCard({t}:{t:Teacher}){
  return <Link href={`/gv/${t.slug}`} className="leaderCard">
    ...
    <span className="leaderInfo">
      ...
      <small>{t.role==='Hiệu trưởng'?'Quản lý chung · Chỉ đạo chuyên môn':`${t.subject} · ${t.team}`}</small>
    </span>
...
export default function HomePage(){
  ...
  const subjects=useMemo(()=>Array.from(new Set(teachers.map(t=>t.subject))).sort((a,b)=>a.localeCompare(b,'vi')),[ ]);
  const filtered=useMemo(()=>{
    ...
    const arr=teachers.filter(t=>{
      ...
      const matchesQuery=!q||norm(`${t.fullName} ${t.key} ${t.subject} ${t.team} ${t.role||''}`).includes(q);
  ...
  const renderTeam=(teamKey:'KHTN'|'KHXH',title:string,order:string[])=>{
    ...
    const extras=Array.from(new Set(teamTeachers.map(t=>t.subject).filter(s=>!order.includes(s))));
    return <section className={`orgSection ${teamKey==='KHTN'?'natural':'social'}`}>
      ...
      <div className="subjectRows">
        {[...order,...extras].map(s=>{
          const list=teamTeachers.filter(t=>t.subject===s); if(!list.length)return null;
  ...
  return <main className="portalShell">
    ...
    <section className="portalHelp"><div><b>ⓘ Hướng dẫn sử dụng</b><span>Chọn giáo viên để mở đúng app cá nhân. Hệ thống sẽ tự nhận diện giáo viên và nguồn PPCT tương ứng.</span></div><Link href="/admin">Quản trị cấu hình →</Link></section>
  </main>
}
