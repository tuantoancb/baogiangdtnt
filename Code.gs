...
// ============================================================================
// V4.177 — CHUẨN HÓA 3 LUỒNG PPCT: CHÍNH KHÓA / CĐ / PĐ
// Chốt 05/09/2026:
// - Tên môn thường => chính khóa (regular)
// - Hậu tố CĐ / CD / Chuyên đề => chuyên đề (elective)
// - Hậu tố PĐ / PD / Phụ đạo => phụ đạo (remedial)
// - Mỗi luồng có bộ đếm PPCT độc lập theo Lớp + Môn.
// - Tích hợp PPCT Phụ đạo Toán 10/11/12: 18 tiết/lớp (9 HKI + 9 HKII).
// ============================================================================

const V4177_TRACK_REGULAR = 'regular';
const V4177_TRACK_ELECTIVE = 'elective';
const V4177_TRACK_REMEDIAL = 'remedial';

function v4135SubjectTrackInfo_(subject) {
  const raw = normalizeText_(subject);
  let base = raw;
  let track = V4177_TRACK_REGULAR;

  // Ưu tiên nhận diện PĐ trước CĐ để không nhập nhằng hậu tố.
  if (/\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i.test(base)) {
    track = V4177_TRACK_REMEDIAL;
    base = normalizeText_(base.replace(/\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i, ''));
  } else if (/\s+(?:CĐ|CD|CHUYÊN\s*ĐỀ|CHUYEN\s*DE)\s*$/i.test(base)) {
    track = V4177_TRACK_ELECTIVE;
    base = normalizeText_(base.replace(/\s+(?:CĐ|CD|CHUYÊN\s*ĐỀ|CHUYEN\s*DE)\s*$/i, ''));
  }
  ...
  if (!base) base = v4157CanonicalBaseSubject_(raw) || raw || 'Chưa xác định môn';
  const displayBase = base === 'GDKTPL' ? 'GDKT&PL' : base;
  const suffix = track === V4177_TRACK_ELECTIVE ? ' CĐ' : (track === V4177_TRACK_REMEDIAL ? ' PĐ' : '');
  const prefix = track === V4177_TRACK_ELECTIVE ? 'CĐ' : (track === V4177_TRACK_REMEDIAL ? 'PĐ' : '');

  return {
    raw: raw,
    baseSubject: base,
    track: track,
    isElective: track === V4177_TRACK_ELECTIVE,
    isRemedial: track === V4177_TRACK_REMEDIAL,
...
function v4135TrackFromPpct_(value) {
  const txt = normalizeText_(value).toUpperCase().replace(/\s+/g, '');
  if (/^(?:PĐ|PD)\d+$/.test(txt)) return V4177_TRACK_REMEDIAL;
  if (/^(?:CĐ|CD)\d+$/.test(txt)) return V4177_TRACK_ELECTIVE;
  return V4177_TRACK_REGULAR;
}
...
function v4135PpctLabel_(n, track) {
  ...
  if (!Number.isFinite(n) || n < 1) return '';
  n = Math.floor(n);
  if (track === V4177_TRACK_ELECTIVE) return 'CĐ' + n;
  if (track === V4177_TRACK_REMEDIAL) return 'PĐ' + n;
  return n;
}
...
function v4127PairKey_(className, subject, track) {
  ...
  const clsKey = keyText_(className || '').replace(/[^a-z0-9]+/g, '');
  const baseKey = v4127SubjectKey_(info.baseSubject);
  const suffix = actualTrack === V4177_TRACK_ELECTIVE ? '||cd' : (actualTrack === V4177_TRACK_REMEDIAL ? '||pd' : '');
  return clsKey + '||' + baseKey + suffix;
}

// PPCT Phụ đạo Toán do người dùng cung cấp ngày 05/09/2026.
const V4177_MATH_REMEDIAL_MAP = {
  '10': {
    '1':'Mệnh đề',
    ...
    '7':'Tổng, hiệu và tích của vectơ với một số',
    '8':'Vectơ trong mặt phẳng tọa độ và tích vô hướng',
    '9':'Ôn tập tổng hợp học kì I',
    '10':'Hàm số',
    '11':'Hàm số bậc hai',
    '12':'Dấu của tam thức bậc hai',
    '13':'Phương trình quy về phương trình bậc hai',
    '14':'Phương trình đường thẳng',
    '15':'Vị trí tương đối, góc và khoảng cách giữa hai đường thẳng',
    '16':'Đường tròn và ba đường conic',
    '17':'Quy tắc đếm, hoán vị, chỉnh hợp, tổ hợp và nhị thức Newton',
    '18':'Biến cố và xác suất theo định nghĩa cổ điển'
  },
  '11': {
    ...
    '7':'Các số đặc trưng đo xu thế trung tâm cho mẫu số liệu ghép nhóm',
    '8':'Quan hệ song song trong không gian',
    '9':'Giới hạn và hàm số liên tục',
    '10':'Lũy thừa với số mũ thực',
    '11':'Lôgarit',
    '12':'Hàm số mũ và hàm số lôgarit',
    '13':'Phương trình, bất phương trình mũ và lôgarit',
    '14':'Đường thẳng vuông góc với mặt phẳng và hai mặt phẳng vuông góc',
    '15':'Góc, khoảng cách trong không gian',
    '16':'Thể tích',
    '17':'Xác suất: biến cố hợp, giao; công thức cộng và nhân',
    '18':'Đạo hàm và các quy tắc tính đạo hàm'
  },
  '12': {
    ...
    '5':'Khảo sát và vẽ đồ thị một số hàm số cơ bản',
    '6':'Ứng dụng đạo hàm vào bài toán thực tiễn',
    '7':'Vectơ và các phép toán trong không gian',
    '8':'Hệ tọa độ và biểu thức tọa độ trong không gian',
    '9':'Các số đặc trưng đo mức độ phân tán của mẫu số liệu ghép nhóm',
    '10':'Nguyên hàm',
    '11':'Tích phân',
    '12':'Ứng dụng hình học của tích phân',
    '13':'Phương trình mặt phẳng',
    '14':'Phương trình đường thẳng trong không gian',
    '15':'Công thức tính góc trong không gian',
    '16':'Phương trình mặt cầu',
    '17':'Xác suất có điều kiện',
    '18':'Công thức xác suất toàn phần và công thức Bayes'
  }
};

// Gắn nguồn PĐ Toán vào Kho PPCT chuẩn hiện có mà không làm thay đổi chính khóa/CĐ.
(function v4177InstallMathRemedialWarehouse_(){
  try {
    const math = V4151_PPCT_WAREHOUSE && V4151_PPCT_WAREHOUSE['Toán'];
    if (!math) return;
    math.remedialMap = V4177_MATH_REMEDIAL_MAP;
    ...
    console.log('V4.177 install remedial warehouse: ' + (e && e.message ? e.message : e));
  }
})();
...
function v4151WarehouseCurriculum_(subject) {
  ...
  return {
    map:src.map || {},
    electiveMap:src.electiveMap || {},
    remedialMap:src.remedialMap || {},
    configured:true,
    entries:Number(src.entries||0),
    regularEntries:Number(src.regularEntries||0),
    electiveEntries:Number(src.electiveEntries||0),
    remedialEntries:Number(src.remedialEntries||0),
    kind:'Kho PPCT chuẩn', title:'Nguồn chuẩn của trường · ' + canonical,
    error:'', subject:canonical,
    gradeCounts:src.gradeCounts || {},
    electiveGradeCounts:src.electiveGradeCounts || {},
    remedialGradeCounts:src.remedialGradeCounts || {},
    format:src.format || 'Tích hợp sẵn từ Kế hoạch dạy học 2026–2027',
    warnings:src.warnings || [], warningCount:Number(src.warningCount||0), criticalWarnings:src.criticalWarnings||[], criticalWarningCount:Number(src.criticalWarningCount||0), partial:!!src.partial,
    sourceType:'school', isOverride:false, defaultAvailable:true,
    sourceFile:src.title || '', warehouseVersion:'2026-09-05'
  };
}

// Giữ dữ liệu PĐ Toán của kho trường khi không có nguồn cá nhân.
// Nếu có nguồn cá nhân, parser cũ vẫn phục vụ chính khóa/CĐ; PĐ sẽ dùng kho trường khi nguồn cá nhân không có remedialMap.
const v4177LoadPpctCurriculumBase_ = loadPpctCurriculum_;
loadPpctCurriculum_ = function(subject) {
  const canonical = v4151CanonicalSubject_(subject);
  const data = v4177LoadPpctCurriculumBase_(canonical) || {};
  const school = v4151WarehouseCurriculum_(canonical);
...
function externalLessonFor_(curriculum, grade, ppct, track) {
  ...
  if (actualTrack === V4177_TRACK_ELECTIVE) root = curriculum && curriculum.electiveMap;
  else if (actualTrack === V4177_TRACK_REMEDIAL) root = curriculum && curriculum.remedialMap;
  const byGrade = root && root[String(grade)];
  return byGrade && byGrade[String(n)] ? normalizeText_(byGrade[String(n)]) : '';
}
...
function getPpctSourceConfig(subject) {
  ...
  return {
    subject:canonical,
    url:url,
    configured:!!(url || school),
    defaultAvailable:!!school,
    isOverride:!!url,
    entries:Number(data.entries||0),
    regularEntries:Number(data.regularEntries||0),
    electiveEntries:Number(data.electiveEntries||0),
    remedialEntries:Number(data.remedialEntries||0),
    kind:data.kind || (school ? 'Kho PPCT chuẩn' : ''),
    title:data.title || (school ? ('Nguồn chuẩn của trường · '+canonical) : ''),
    error:data.error || '',
    gradeCounts:data.gradeCounts || {},
    electiveGradeCounts:data.electiveGradeCounts || {},
    remedialGradeCounts:data.remedialGradeCounts || {},
    format:data.format || '',
    warnings:data.warnings || [],
    warningCount:Number(data.warningCount||0),
    warehouseVersion:'2026-09-05',
    tracks:{regular:true,elective:Number(data.electiveEntries||0)>0,remedial:Number(data.remedialEntries||0)>0}
  };
}

// Đọc ngược Lịch báo giảng: nhận PĐ độc lập, không biến thành chính khóa.
function readTeacherLessonRows_(sheet, teacherKey, mondayYmd) {
  const fullName = TEACHER_MAP[teacherKey];
  if (!fullName) throw new Error('Giáo viên không hợp lệ.');
  const blockStartZero = findTeacherBlockStart_(sheet, fullName);
  ...
  for (let r = 0; r < vals.length; r++) {
    ...
    const subjectInfo = v4135SubjectTrackInfo_(rawSubject);
    const ppctTrack = v4135TrackFromPpct_(rawPpct);
    const track = ppctTrack !== V4177_TRACK_REGULAR ? ppctTrack : subjectInfo.track;
    const baseSubject = subjectInfo.baseSubject;
    const subject = track === V4177_TRACK_ELECTIVE ? (baseSubject + ' CĐ') : (track === V4177_TRACK_REMEDIAL ? (baseSubject + ' PĐ') : baseSubject);
    const cls = normalizeText_(row[3]);
    if (!/^(10|11|12)[A-Za-z0-9]+$/i.test(cls)) continue;
    if (!Number.isFinite(period) || period < 1 || period > 5) continue;
    ...
    rows.push({
      row:r + 1, blockStartZero:blockStartZero, startCol1:startCol1,
      ppctCol:startCol1 + 4, lessonCol:startCol1 + 5,
      className:cls, subject:subject, baseSubject:baseSubject, track:track,
      isElective:track === V4177_TRACK_ELECTIVE, isRemedial:track === V4177_TRACK_REMEDIAL,
      pairKey:v4127PairKey_(cls, subject, track), grade:Number(String(cls).slice(0, 2)),
      oldPpct:oldPpct, oldPpctLabel:rawPpct, oldLesson:normalizeText_(row[5]),
      dayNum:currentDay, dateShort:dateShort, session:currentSession, period:period
    });
...
// Gợi ý đầu PPCT: trả thêm thông tin PĐ để UI hiện đúng PĐ1…
const v4177GetClassPpctSuggestionsBase_ = getClassPpctSuggestions;
getClassPpctSuggestions = function(teacherKey, reportSheetName, tkbSheetName) {
  const res = v4177GetClassPpctSuggestionsBase_(teacherKey, reportSheetName, tkbSheetName) || {};
  (res.classes || []).forEach(x => {
    const info = v4135SubjectTrackInfo_(x.subject || x.baseSubject || '');
    x.track = x.track || info.track;
    x.isElective = x.track === V4177_TRACK_ELECTIVE;
    x.isRemedial = x.track === V4177_TRACK_REMEDIAL;
    x.ppctPrefix = x.track === V4177_TRACK_ELECTIVE ? 'CĐ' : (x.track === V4177_TRACK_REMEDIAL ? 'PĐ' : '');
  });
  return res;
};

// Tiến độ tổng hợp hiện tại của trường chỉ có cột chính khóa/CĐ.
// PĐ vẫn được ghi đầy đủ vào Lịch báo giảng và có PPCT riêng; bỏ qua PĐ khi đẩy sang sheet Tiến độ
// để không ghi nhầm vào cột chính khóa cho đến khi sheet Tiến độ có cột PĐ riêng.
const v4177BuildProgressPairsBase_ = (typeof v4158BuildProgressPairs_ === 'function') ? v4158BuildProgressPairs_ : null;
if (v4177BuildProgressPairsBase_) {
  v4158BuildProgressPairs_ = function(records) {
    const safe = (records || []).filter(r => {
      const info = v4135SubjectTrackInfo_((r && (r.subject || r.baseSubject)) || '');
      const t = v4135TrackFromPpct_(r && r.ppct);
      return info.track !== V4177_TRACK_REMEDIAL && t !== V4177_TRACK_REMEDIAL && (r && r.track) !== V4177_TRACK_REMEDIAL;
    });
    return v4177BuildProgressPairsBase_(safe);
  };
}


// V4.177: loại PĐ khỏi sheet Tiến độ tổng hợp cũ (sheet này chưa có cột PĐ riêng).
function v4158ProgressPairsFromRecords_(records) {
  const pairs = {};
  (records || []).forEach(r => {
    const cls = normalizeText_(r && r.className).replace(/\s+/g, '');
    ...
    const base = v4157CanonicalBaseSubject_(info.baseSubject || info.displaySubject || (r && r.subject) || '');
    if (!base) return;
    const fromPpct = v4135TrackFromPpct_(r && r.ppct);
    const track = fromPpct !== V4177_TRACK_REGULAR ? fromPpct : ((r && r.track) || info.track || V4177_TRACK_REGULAR);
    if (track === V4177_TRACK_REMEDIAL) return;
    const n = Number(r && r.ppctNumber) > 0 ? Math.floor(Number(r.ppctNumber)) : v4135PpctNumber_(r && r.ppct);
    if (!n) return;
    const key = cls.toLowerCase() + '|' + v4127SubjectKey_(base) + '|' + track;
    if (!pairs[key] || n > pairs[key].ppct) {
      pairs[key] = {key:key,className:cls,baseSubject:base,subjectKey:v4127SubjectKey_(base),track:track,ppct:n};
    }
  });
  return Object.keys(pairs).map(k => pairs[k]);
}
...
// ============================================================================
// V4.178 — TIẾN ĐỘ 3 KHU ĐỘC LẬP: CHÍNH KHÓA / CHUYÊN ĐỀ / PHỤ ĐẠO ĐẠI TRÀ
// Chốt 05/09/2026 theo cấu trúc thật của file:
// DTNT - Bao cao tien do thuc hien chuong trinh 2026 - 2027
// - Chính khóa: Môn/HĐGD bắt buộc + Môn học lựa chọn
// - CĐ: khu "Chuyên đề"
// - PĐ: khu "Phụ đạo đại trà"
// - Không ghi PĐ sang "Phụ đạo trước kiểm tra" hoặc các khu bổ trợ khác.
// ============================================================================

const V4178_PROGRESS_VERSION = '4.178';

// Đọc 2 hàng tiêu đề và phân cột theo đúng NHÓM đang bao phủ từng cột.
// Cách này không phụ thuộc cột cố định nên vẫn an toàn nếu trường chèn/xóa môn.
function v4158ProgressColumnMap_(sheet) {
  ...
  const remedial = {};
  let currentGroup = '';
  let electiveStart = -1, electiveEnd = -1, remedialStart = -1, remedialEnd = -1;
  ...
  for (let c = 1; c <= lastCol; c++) {
    ...
    if (currentGroup === 'elective') {
      if (electiveStart < 0) electiveStart = c;
      electiveEnd = c + 1;
    } else if (currentGroup === 'remedial') {
      if (remedialStart < 0) remedialStart = c;
      remedialEnd = c + 1;
    }

    if (c < 4) continue; // A-C = Tuần / TT / Lớp
    const base = v4158ProgressHeaderSubject_(subjectRow[c - 1]);
    if (!base) continue;
    const sk = v4127SubjectKey_(base);
    if (!sk) continue;
    ...
    if (currentGroup === 'elective') {
      ...
      if (remedial[sk] == null) remedial[sk] = c;
    } else if (currentGroup === 'regular') {
      if (regular[sk] == null) regular[sk] = c;
    }
  }
  ...
  return {
    ...
    elective:elective,
    remedial:remedial,
    electiveStart:electiveStart,
    electiveEnd:electiveEnd,
...
function v4158ProgressPairsFromRecords_(records) {
  ...
  (records || []).forEach(r => {
    ...
    const ppctTrack = v4135TrackFromPpct_(r && r.ppct);
    let track = (r && r.track) || info.track || V4177_TRACK_REGULAR;
    // Prefix trong cột PPCT là tín hiệu mạnh nhất nếu có.
    if (ppctTrack !== V4177_TRACK_REGULAR) track = ppctTrack;
    if (track !== V4177_TRACK_ELECTIVE && track !== V4177_TRACK_REMEDIAL) track = V4177_TRACK_REGULAR;

    const n = Number(r && r.ppctNumber) > 0
      ? Math.floor(Number(r.ppctNumber))
      : v4135PpctNumber_(r && r.ppct);
    if (!n) return;
...
function v4178ProgressMapForTrack_(colMap, track) {
  if (track === V4177_TRACK_ELECTIVE) return colMap.elective || {};
  if (track === V4177_TRACK_REMEDIAL) return colMap.remedial || {};
  return colMap.regular || {};
}

function v4178ProgressTrackLabel_(track) {
  if (track === V4177_TRACK_ELECTIVE) return 'Chuyên đề';
  if (track === V4177_TRACK_REMEDIAL) return 'Phụ đạo đại trà';
  return 'Chính khóa';
}
...
function updateProgressReport_(payload, records) {
  ...
  Object.keys(groups).forEach(sheetName => {
    ...
    groups[sheetName].forEach(p => {
      ...
      const map = v4178ProgressMapForTrack_(colMap, p.track);
      const targetCol = map[p.subjectKey];
      if (!targetCol) {
        throw new Error(
          'Không tìm thấy cột ' + p.baseSubject + ' trong khu ' +
          v4178ProgressTrackLabel_(p.track) + ' của sheet ' + sheetName + '.'
        );
      }
      ...
      details.push({
        ...
        subject:p.baseSubject,
        track:p.track,
        trackLabel:v4178ProgressTrackLabel_(p.track),
        week:week,
        ppct:p.ppct,
        range:range.getA1Notation()
      });
  ...
  console.log('V4.178 updateProgressReport_ completed in ' + (Date.now() - startedAt) + ' ms; pairs=' + details.length);
  return details;
}
...
function kiemTraTienDoDaCapNhat(payload) {
  try {
    ...
    for (const p of pairs) {
      ...
      const x = cache[sheetName];
      const row = x.rows[p.className.toLowerCase()];
      const col = v4178ProgressMapForTrack_(x.cols, p.track)[p.subjectKey];
      if (!row || !col) return {ok:true, done:false, pairs:matched};
...
// Khi quản trị yêu cầu dựng lại một tuần, chỉ dọn 3 khu mà app đang quản lý.
// Không đụng "Phụ đạo trước kiểm tra", "Bổ trợ" hay "Phụ đạo HS Lô lô".
function v4158ClearCoreProgressWeek_(week) {
  const ss = SpreadsheetApp.openById(TIEN_DO_SPREADSHEET_ID);
  const cleared = [];
  ['Lop 10','Lop 11','Lop 12'].forEach(sheetName => {
    ...
    if (!sh) return;
    const rowMap = v4158ProgressRowMap_(sh, week);
    const maps = v4158ProgressColumnMap_(sh);
    const cols = [];
    [maps.regular, maps.elective, maps.remedial].forEach(m => {
      Object.keys(m || {}).forEach(k => cols.push(Number(m[k])));
    });
    const uniqueCols = Array.from(new Set(cols.filter(c => Number.isFinite(c) && c > 0))).sort((a,b)=>a-b);
    Object.keys(rowMap).forEach(cls => {
      const row = rowMap[cls];
      uniqueCols.forEach(col => sh.getRange(row, col).clearContent());
      cleared.push(sheetName + '!' + row);
    });
  });
  return cleared;
...
// Hàm kiểm tra nhanh dành cho quản trị sau khi dán Code.gs mới.
function V4178_KIEM_TRA_3_KHU_TIEN_DO() {
  const ss = SpreadsheetApp.openById(TIEN_DO_SPREADSHEET_ID);
  const result = {};
  ['Lop 10','Lop 11','Lop 12'].forEach(name => {
    const sh = ss.getSheetByName(name);
    if (!sh) return;
    const maps = v4158ProgressColumnMap_(sh);
    result[name] = {
      chinhKhoa:Object.keys(maps.regular || {}).length,
      chuyenDe:Object.keys(maps.elective || {}).length,
      phuDaoDaiTra:Object.keys(maps.remedial || {}).length,
      coToanChinhKhoa:!!(maps.regular || {})[v4127SubjectKey_('Toán')],
      coToanChuyenDe:!!(maps.elective || {})[v4127SubjectKey_('Toán')],
      coToanPhuDao:!!(maps.remedial || {})[v4127SubjectKey_('Toán')]
    };
  });
  return {ok:true, version:V4178_PROGRESS_VERSION, sheets:result};
}


// ============================================================================
// V4.179 — BẬT / TẮT “PĐ → CHÍNH KHÓA” THEO TỪNG GIÁO VIÊN
// Nền: V4.178 (3 luồng chính khóa / CĐ / PĐ)
// - Mặc định TẮT.
// - BẬT: tiết PĐ trên TKB được đổi thành môn chính khóa trước khi tính PPCT,
//        vì vậy lấy CT kế tiếp và không dùng luồng PĐ độc lập.
// - TẮT: tiết PĐ tạm bỏ qua khỏi lịch hiệu lực, không chiếm CT.
// - Trạng thái lưu riêng theo từng giáo viên.
// ============================================================================
const V4179_PD_AS_REGULAR_PREFIX = 'V4179_PD_AS_REGULAR_';

function v4179TeacherKey_(teacherRef) {
  const raw = normalizeText_(teacherRef);
  try { return resolveTeacherKey_(raw || FIXED_TEACHER_KEY); } catch (e) {}
  if (typeof TEACHER_MAP !== 'undefined' && TEACHER_MAP[raw]) return raw;
  const profiles = (typeof TEACHER_PROFILES !== 'undefined' && TEACHER_PROFILES) || {};
  const bySlug = Object.keys(profiles).find(k => normalizeText_(profiles[k] && profiles[k].slug) === raw);
  return bySlug || (typeof FIXED_TEACHER_KEY !== 'undefined' ? FIXED_TEACHER_KEY : raw);
}

function v4179PdPropertyKey_(teacherRef) {
  const key = v4179TeacherKey_(teacherRef);
  return V4179_PD_AS_REGULAR_PREFIX + teacherSlug_(key);
}

function v4179PdAsRegularEnabled_(teacherRef) {
  const v = PropertiesService.getScriptProperties().getProperty(v4179PdPropertyKey_(teacherRef));
  const s = String(v || '').toLowerCase();
  return s === '1' || s === 'true' || s === 'on' || s === 'yes';
}

function getPdAsRegularConfig(teacherRef) {
  const teacherKey = v4179TeacherKey_(teacherRef);
  const profiles = (typeof TEACHER_PROFILES !== 'undefined' && TEACHER_PROFILES) || {};
  const profile = profiles[teacherKey] || {fullName:(typeof TEACHER_MAP !== 'undefined' && TEACHER_MAP[teacherKey]) || teacherKey, slug:teacherSlug_(teacherKey)};
  return {
    ok:true,
    teacherKey:teacherKey,
    teacherName:profile.fullName || ((typeof TEACHER_MAP !== 'undefined' && TEACHER_MAP[teacherKey]) || teacherKey),
    teacherSlug:profile.slug || teacherSlug_(teacherKey),
    enabled:v4179PdAsRegularEnabled_(teacherKey),
    label:'PĐ → Chính khóa',
    enabledText:'Bật: tiết PĐ được tính như chính khóa và lấy CT kế tiếp.',
    disabledText:'Tắt: tiết PĐ tạm bỏ qua, không chiếm CT chính khóa.'
  };
}

function savePdAsRegularConfig(teacherRef, enabled) {
  const teacherKey = v4179TeacherKey_(teacherRef);
  const on = enabled === true || enabled === 1 || ['1','true','on','yes'].indexOf(String(enabled).toLowerCase()) >= 0;
  const props = PropertiesService.getScriptProperties();
  const propKey = v4179PdPropertyKey_(teacherKey);
  if (on) props.setProperty(propKey, '1');
  else props.deleteProperty(propKey);
  return getPdAsRegularConfig(teacherKey);
}

function v4179IsPdSubject_(subject) {
  const s = normalizeText_(subject);
  return /\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i.test(s);
}

function v4179StripPdSuffix_(subject) {
  return normalizeText_(normalizeText_(subject).replace(/\s+(?:PĐ|PD|PHỤ\s*ĐẠO|PHU\s*DAO)\s*$/i, ''));
}

// Ghi đè hàm lịch hiệu lực cuối cùng của V4.178.
// Hàm gốc V4.178 đã áp dụng: TKB -> dạy bù -> hoán đổi -> phát sinh -> ngày nghỉ.
// Bản V4.179 chỉ thêm bước xử lý PĐ theo công tắc của đúng giáo viên.
function v4136ReadEffectiveSchedule_(teacherKey, tkbSheetName) {
  const base = v4155FilterDayOff_(v4155ScheduleBeforeDayOff_(teacherKey, tkbSheetName), tkbSheetName);
  const on = v4179PdAsRegularEnabled_(teacherKey);
  const out = [];
  (base || []).forEach(rec => {
    const isPd = v4179IsPdSubject_(rec && rec.subject);
    if (!isPd) {
      out.push(rec);
      return;
    }
    if (!on) return; // TẮT: bỏ qua PĐ, không chiếm CT
    const copy = Object.assign({}, rec);
    copy.originalSubject = rec.subject;
    copy.subject = v4179StripPdSuffix_(rec.subject); // BẬT: biến thành chính khóa
    copy.pdAsRegular = true;
    copy.pdMode = 'regular';
    out.push(copy);
  });
  return out;
...
// Gắn trạng thái vào dữ liệu khởi tạo để frontend có thể render công tắc ngay.
const V4179_getInitialData_original = getInitialData;
getInitialData = function(teacherRef) {
  const data = V4179_getInitialData_original(teacherRef);
  const teacherKey = (data && data.activeTeacherKey) || v4179TeacherKey_(teacherRef);
  const cfg = getPdAsRegularConfig(teacherKey);
  data.pdAsRegular = cfg;
  data.pdAsRegularEnabled = !!cfg.enabled;
  return data;
};

// Mở API cho frontend Vercel / Apps Script.
if (typeof VERCEL_API_ACTIONS !== 'undefined') {
  VERCEL_API_ACTIONS.getPdAsRegularConfig = getPdAsRegularConfig;
  VERCEL_API_ACTIONS.savePdAsRegularConfig = savePdAsRegularConfig;
}
